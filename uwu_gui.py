...
class TextLogger(ttk.Frame):
    def __init__(self, master, **kw):
        ...
        self.text=tk.Text(self,height=14,wrap="word",bg="#0f1117",fg="#d1e7ff",insertbackground="#d1e7ff")
        self.text.config(state="disabled"); self.text.pack(fill="both",expand=True)
        self.queue=queue.Queue(); self.after(200,self._poll)
    def log(self,s):
...
class ScriptRunner:
    def __init__(self, logger:TextLogger):
        self.logger=logger; self.proc=None; self.q=queue.Queue(); self._polling=False
    def run(self, cmd, workdir="/var/www/html"):
        if self.proc: return messagebox.showwarning("Déjà en cours","Un script tourne déjà.")
        self.logger.log(f"Exec {shlex.join(cmd)}")
        def _reader():
            try:
                self.proc=subprocess.Popen(cmd,cwd=workdir,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
        ...
        if not self._polling: self._polling=True; self._drain()
    def _drain(self):
        try:
            while True:
                it=self.q.get_nowait()
                if isinstance(it,tuple) and it[0]=="__EXIT__":
                    self.logger.log(f"[done] rc={it[1]}"); self.proc=None; self._polling=False; return
                else: self.logger.log(it)
        except queue.Empty: pass
        if self.proc: self.logger.after(80,self._drain)
        else: self._polling=False

# ---------- GUI ----------

class UwUGui(ttk.Frame):
    def __init__(self, master):
        super().__init__(master,padding=8); self.pack(fill="both",expand=True)
        self.selected_log=tk.StringVar(value=str(LOG_CANDIDATES[0]) if LOG_CANDIDATES else "")
        self.runner=None; self._build_layout(); self._start_stats()
    ...
    def _build_layout(self):
        # Stats
        stats=self._frame_card(self,"Stats système"); stats.pack(fill="x",pady=4)
        ...
        # Services
        svc=self._frame_card(self,"Services systemd"); svc.pack(fill="x",pady=4)
        ...
        # Script
        scr=self._frame_card(self,"Script logs_auto.py"); scr.pack(fill="x",pady=4)
        ttk.Button(scr,text="Lancer logs_auto.py",command=self.run_logs_auto).pack(side="left",padx=4)

        # Cron
        cron=self._frame_card(self,"Cron"); cron.pack(fill="x",pady=4)
        ...
        # Logs
        logf=self._frame_card(self,"Logs"); logf.pack(fill="both",expand=True,pady=4)
        top=tk.Frame(logf); top.pack(fill="x")
        self.cmb_log=ttk.Combobox(top,textvariable=self.selected_log,values=[str(p) for p in LOG_CANDIDATES],width=60)
        self.cmb_log.pack(side="left",padx=4)
        ttk.Button(top,text="Tail 200",command=self.tail_log).pack(side="left",padx=4)
        self.log_view=TextLogger(logf); self.log_view.pack(fill="both",expand=True)
        self.runner=ScriptRunner(self.log_view)

    # Actions
    def do_service(self,name,action):
        rc,out,err=service_cmd(name,action)
        self.log_view.log(f"{name} {action}: {'OK' if rc==0 else 'ERR'} {err or out}")
    ...
    def ensure_cron(self):
        ...
        if changed: self.lbl_cron.config(text="Ajouté ✓"); self.log_view.log(f"Cron ajouté: {line}")
        elif changed is None: self.lbl_cron.config(text="Déjà présent ✓"); self.log_view.log("Cron: déjà présent")
        else: self.lbl_cron.config(text="Erreur"); messagebox.showerror("Cron",err)

    def tail_log(self):
        p=Path(self.selected_log.get())
        if not p.exists(): return messagebox.showerror("Log",f"Introuvable: {p}")
    ...
    def _popup(self,title,content):
        ...
        txt=tk.Text(win,width=100,height=30); txt.pack(fill="both",expand=True)
    ...
    def _loop(self):
        ...
        while True:
            try:
                ...
                else: ru,rt,rp=get_meminfo_fallback()
                if psutil:
                    du=psutil.disk_usage("/"); u,t,p=du.used,du.total,du.percent
                else: u,t,p=get_disk_usage("/")
...
def main():
    root=tk.Tk(); root.title("UwU Logs – Dashboard")
    root.geometry("1000x700"); UwUGui(root); root.mainloop()
